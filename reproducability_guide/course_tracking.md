...
### Component 5: Sidebar Navigation & Learning Space Registration

To surface the Course Page in the main DeepTutor frontend navigation:

#### [MODIFY] [SidebarShell.tsx](../web/components/sidebar/SidebarShell.tsx)
Add the Courses route to `PRIMARY_NAV` in `SidebarShell.tsx`:

```tsx
import { GraduationCap } from "lucide-react";

const PRIMARY_NAV: NavEntry[] = [
  { href: "/home", label: "Home", icon: House, requires: "llm" },
  { href: "/courses/cs101", label: "Courses", icon: GraduationCap, tooltipKey: "Courses tooltip" },
  { href: "/partners", label: "Partners", icon: HeartHandshake, requires: "llm" },
];
```

#### [MODIFY] [SpaceDashboard.tsx](../web/components/space/SpaceDashboard.tsx)
Add the Courses tile to `GROUPS` in `SpaceDashboard.tsx`:
...
```tsx
{
  ...
  blurb: { zh: "学习课程视频、进行诊断测试与随堂测验。", en: "Study course lectures, diagnostic quizzes, and reflection essays." },
...
| **6** | **Blank Course Page Render** | Initial blueprint snippet contained placeholder stubs (`{/* Video Player ... */}`). | Fully implemented `CourseViewerPage` in `web/app/(workspace)/courses/[courseId]/page.tsx` with video, MCQ, essay, and milestone telemetry hooks. |
| **7** | **Course Page Inaccessible / Missing Navigation Button** | Route was missing from sidebar navigation. | Added `/courses/cs101` entry with `GraduationCap` icon to `PRIMARY_NAV` in `SidebarShell.tsx` and `GROUPS` in `SpaceDashboard.tsx`. |
