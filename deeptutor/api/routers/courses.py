...
async def evaluate_quiz(course_id: str, req: EvaluateQuizRequest):
    ...
    if req.question_type == "mcq":
        ...
        eval_prompt = f"""You are a strict academic evaluator. Grade the student's short essay response based on the question and grading rubric.
...
{{
  ...
  "feedback": "1-2 sentences of helpful feedback explaining why the response meets or fails the rubric"
}}"""
        try:
            llm_res = await asyncio.wait_for(complete(eval_prompt), timeout=60.0)
            ...
            print("LLM essay evaluation error/timeout:", exc)
            score = 0.0
            correct = False
            feedback = "AI evaluation could not be completed in time. Please try again."
